...
function handleWheelScroll(upOrDown: number) {
    tableListCopy.forEach(item => {
        ...
        if(!isBar){
            item.startXPos +=scrollCount
            speedCount = scrollCount
        }
        item.startXPos += (isBar ? 0 :scrollCount)
        if (upOrDown > 0) {
...
function watchScroll(scrollDiraction: Ref<number>) {
    watch(() => scrollDiraction.value, newVal => {
        ...
        if (newVal > 0 && (endIndex + 1) >= tableListCopy.length) return YTrans.value = 0; // 上边界
        
        
        scrollYRadio.value = 1- (tableListCopy.length - endIndex)/tableListCopy.length
...
function handleBarYScroll(yBarScrollDis: number) {
    ...
    tableListCopy.forEach(item => {
        ...
        item.startYPos -= yBarScrollDis
    })
    scrollYcount = yBarScrollDis
}

function watchYBar(YTransInfo: {dis:number,flag:boolean}) {
    watch(() => YTransInfo.dis, newVal => {
        if (newVal == 0 || newVal == 1 || !YTransInfo.flag) return
        isBar = true
...
export function useTableBody(
    ...
    return { result, scrollYRadio, YTrans, maxXWidth,initBarRadio: (bodyInfo.endY -  bodyInfo.startY)/scrollHeight}
}