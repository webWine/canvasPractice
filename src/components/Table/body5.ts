...
function renderTarget(list:any[]){
    renderContent(list)
    renderTop()
    renderFooter()
}
...
function watchXBar(XTrans: Ref<number>) {
    ...
    if(newVal == 0) scrollXCount = 0
        ...
        renderTarget(renderList.filter(item=>item.fixed =='left'))
        renderTarget(renderList.filter(item=>item.fixed =='right'))
